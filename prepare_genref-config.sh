...
echo "proteins protein.mol2" > genref-config.txt
echo "ligands ligand_ref.mol2" >> genref-config.txt
echo "outfile ref-results.txt" >> genref-config.txt
echo "residue_number `grep CA PLANTSactiveSiteResidues.mol2 | awk '{print $7}' | paste -s -d" "`" >> genref-config.txt
echo "residue_name `grep CA PLANTSactiveSiteResidues.mol2 | awk '{print $8}' | paste -s -d" "`" >> genref-config.txt
